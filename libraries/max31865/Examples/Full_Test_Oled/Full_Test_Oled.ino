// ****************************************************************************
// MAX31865 Break-out board Full test Example with OLED display
// *****************************************************************************
//
...
// Instantiate the I2C OLED 
U8GLIB_SSD1306_128X64 display(U8G_I2C_OPT_DEV_0|U8G_I2C_OPT_NO_ACK|U8G_I2C_OPT_FAST); // Fast I2C / TWI 

// Macro to calculate where to place text to center it
#define CenterText(t) ( (display.getWidth() - display.getStrWidth(t) ) / 2)

//#define U8G_FONT_SMALL  u8g_font_7x14 
//#define U8G_FONT_SMALL  u8g_font_profont15
//#define U8G_FONT_MEDIUM u8g_font_fub14r
//#define U8G_FONT_BOLD   u8g_font_fub11r
#define U8G_FONT_BOLD   u8g_font_9x15Br
//#define U8G_DIGIT       u8g_font_freedoomr25n
#define U8G_DIGIT       u8g_font_fub25n
...
/* ======================================================================
Function: drawTemp 
Purpose : display temp big digit on OLED page loop
Input   : temp formated string
Output  : - 
Comments: -
====================================================================== */
void drawTemp(char * temp)
{
  display.setFont(U8G_FONT_BOLD);
  display.setFontPosTop();

  // Temp with big digit
  display.setFont(U8G_DIGIT);
  display.setFontPosTop();
  display.drawStr( CenterText(temp), 0, temp);

  // R Ref and R measured values
  display.setFont(U8G_FONT_BOLD);
  display.setFontPosTop();

  sprintf_P(buffer, PSTR("RTD %d Ohms"), rtd.raw_resistance());
  display.drawStr( CenterText(buffer), 32, buffer);

  sprintf_P(buffer, PSTR("REF %d Ohms"), rtd.cfg_rref());
  display.drawStr( CenterText(buffer), 48, buffer);
}

/* ======================================================================
Function: drawError 
Purpose : display error OLED page loop
Input   : status register of MAX31865
Output  : - 
Comments: -
====================================================================== */
void drawError(uint8_t status)
{
  display.setFont(U8G_FONT_BOLD);
  display.setFontPosTop();
  
  if (max31865_state==MAX31865_STATE_NOT_FOUND)
  {
    strcpy_P(buffer, PSTR("Unable to"));
    display.drawStr( CenterText(buffer), 0, buffer);
    strcpy_P(buffer, PSTR("Communicate"));
    display.drawStr( CenterText(buffer),16, buffer);
    strcpy_P(buffer, PSTR("with MAX31865"));
    display.drawStr( CenterText(buffer),32, buffer);
    strcpy_P(buffer, PSTR("chip plugged ?"));
    display.drawStr( CenterText(buffer),48, buffer);
  }
  else if (max31865_state==MAX31865_STATE_TRANSMIT_ERROR)
  {
    strcpy_P(buffer, PSTR("MAX31868 SPI"));
    display.drawStr( CenterText(buffer), 0, buffer);
    strcpy_P(buffer, PSTR("Communication"));
    display.drawStr( CenterText(buffer),16, buffer);
    strcpy_P(buffer, PSTR("problem"));
    display.drawStr( CenterText(buffer),32, buffer);
    strcpy_P(buffer, PSTR("check wiring!"));
    display.drawStr( CenterText(buffer),48, buffer);
  }
  /*
  else if (max31865_state==MAX31865_RTD_CONFIG_FAULT)
  {
    display.setCursor(8,0);
    display.print("RTD Config");
    display.setCursor(32,2);
    display.print("Error");
    display.setCursor(4,4);
    display.print("Check Solder");
    display.setCursor(4,6);
    display.print("pads config!");

  }*/
  else
  {
    sprintf_P(buffer, PSTR("Status:0x%02X"), status);
    display.drawStr( CenterText(buffer), 0, buffer);
    //display.setPrintPos(0,16);
    sprintf_P(buffer, PSTR("RTD=%d"), rtd.raw_resistance());
    display.drawStr( CenterText(buffer),16, buffer);
    ...
    // Handle new line on OLED
    for (uint8_t i=0; i<status_text.length(); i++) {
      display.print(status_text.charAt(i));

      if ( (128 - display.getPrintCol()) <= 16 ) 
        display.setPrintPos(0, display.getPrintRow()+16 );
    }
  }
}
...
/* ======================================================================
Function: drawSplash 
Purpose : display splash screen page loop
Input   : -
Output  : - 
Comments: -
====================================================================== */
void drawSplash(void)
{
  ...
  strcpy_P(buffer, PSTR("MAX31865"));
  display.drawStr( CenterText(buffer), 0, buffer);
  strcpy_P(buffer, PSTR("Full Tests"));
  display.drawStr( CenterText(buffer),16, buffer);
  strcpy_P(buffer, PSTR(__DATE__));
  display.drawStr( CenterText(buffer),32, buffer);
  strcpy_P(buffer, PSTR(__TIME__));
  display.drawStr( CenterText(buffer),48, buffer);
}
...
void setup()
{
  ...
  // ===========================
  // initialize with the default I2C addr 
  // depending on display it can be different, check your doc
  display.firstPage(); 
  do { 
    drawSplash();
  } 
